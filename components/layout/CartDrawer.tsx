      ...
      {isCartOpen && (
        <>
          ...
          <motion.div
            ...
            className="fixed top-0 right-0 bottom-0 w-full max-w-md bg-ivory z-50 flex flex-col shadow-2xl overflow-hidden border-l border-cream-300"
          >
            {/* Drawer Header */}
            <div className="p-5 border-b border-cream-200 flex items-center justify-between bg-cream-50">
              <div className="flex items-center gap-2">
                ...
                <h2 className="font-serif text-xl font-bold tracking-wide text-forest">
                  Your Ritual Bag
                </h2>
            ...
            {/* Free Shipping Meter */}
            <div className="p-4 bg-cream-100 border-b border-cream-200 space-y-1.5">
              <div className="flex items-center justify-between text-xs font-sans">
                <span className="font-medium text-earth-700 flex items-center gap-1.5">
                  <Sparkles className="w-3.5 h-3.5 text-gold" />
                  {unlocked ? (
                    <span className="text-forest font-semibold">
                      You&apos;ve unlocked Complimentary Carbon-Neutral Shipping!
                    </span>
                  ) : (
                    <span>
                      Add <strong className="text-forest">{formatPrice(remaining)}</strong> more for FREE shipping
                    </span>
                  )}
                </span>
            ...
            <div className="flex-1 overflow-y-auto p-5 space-y-4">
              {items.length === 0 ? (
                ...
                items.map((item) => (
                  <div
                    key={item.id}
                    className="flex gap-4 p-3.5 rounded-xl border border-cream-200 bg-cream-50/50 hover:border-cream-300 transition-all"
                  >
                    {/* Item Image */}
                      ...
                      <div>
                        <div className="flex items-start justify-between gap-2">
                          <Link
                            ...
                            onClick={closeCart}
                            className="font-serif text-base font-semibold text-forest hover:text-sage line-clamp-1 leading-snug"
                          >
                            {item.name}
                          </Link>
                          <button
                        ...
                        <p className="font-sans text-xs text-earth-500 font-medium mt-0.5">
                      ...
                      <div className="flex items-center justify-between pt-2">
                        ...
                        <div className="flex items-center border border-cream-300 rounded-full bg-ivory overflow-hidden">
                          <button
                            ...
                            className="p-1.5 text-earth-600 hover:text-forest hover:bg-cream-100 transition-colors"
                            aria-label="Decrease quantity"
                          >
                            <Minus className="w-3.5 h-3.5" />
                          </button>
                          <span className="font-sans text-xs font-bold text-forest px-2.5 min-w-[20px] text-center">
                            {item.quantity}
                          </span>
                          <button
                            onClick={() => updateQuantity(item.id, item.quantity + 1)}
            ...
            {items.length > 0 && (
              <div className="p-5 border-t border-cream-200 bg-cream-50 space-y-4">
                {/* Promo Code Box */}
                {!promoCode ? (
                  <form onSubmit={handleApplyPromo} className="flex gap-2">
                    <div className="relative flex-1">
                      <input
                        ...
                        className="w-full px-3 py-2 text-xs font-sans rounded-lg border border-cream-300 bg-ivory focus:outline-none focus:border-forest uppercase"
                      />
                    </div>
                    <button
                      type="submit"
                      className="px-4 py-2 bg-cream-200 hover:bg-forest hover:text-ivory text-forest text-xs font-sans font-semibold rounded-lg transition-colors"
                    >
                      Apply
                    </button>
                  </form>
                ) : (
                  <div className="flex items-center justify-between p-2 rounded-lg bg-forest/10 border border-forest/20 text-xs font-sans">
                    ...
                    <button
                      ...
                      className="text-earth-500 hover:text-red-600 font-medium underline text-[11px]"
                    >
                      Remove
                    </button>
                  </div>
                ...
                {/* Calculations */}
                <div className="space-y-1.5 font-sans text-xs text-earth-700">
                  ...
                  <div className="flex justify-between">
                    <span>Shipping</span>
                    <span>{unlocked ? <span className="text-forest font-bold">FREE</span> : formatPrice(4.95)}</span>
                  </div>
                  <div className="flex justify-between text-sm font-bold text-forest pt-2 border-t border-cream-200">
                ...
                <div className="space-y-2 pt-1">
                  <Link
                    href="/checkout"
                    onClick={closeCart}
                    className="w-full flex items-center justify-center gap-2 py-3.5 rounded-full bg-forest text-ivory font-sans text-xs font-bold uppercase tracking-widest hover:bg-forest-700 transition-all shadow-md group"
                  >
                  ...
                  <Link
                    href="/cart"
                    onClick={closeCart}
                    className="w-full block text-center py-2.5 rounded-full border border-cream-300 text-forest font-sans text-xs font-semibold hover:bg-cream-100 transition-colors"
                  >
                    View Full Cart Page
                  </Link>
                </div>
